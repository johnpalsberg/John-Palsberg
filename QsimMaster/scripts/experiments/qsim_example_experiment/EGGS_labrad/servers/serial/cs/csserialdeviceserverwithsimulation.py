...
class CSSerialDeviceServerSim(LabradServer):
    ...
    class SerialConnection(object):
        ...
        """
        def __init__(self, ser, port, simulated, **kwargs):
            # parse kwargs
    ...
    def initSerial(self, serStr, port, **kwargs):
        ...
        if self.simulated:
            print('\tport:\t No port: device simulated")
        else:
            print('\tport:\t%s' % port)
        ...
        try:
            ...
            self.ser = self.SerialConnection(ser=ser, port=port, simulated=self.simulated, **kwargs)
            ...
            if not self.simulated:
                ...
                print('Simulated serial connection opened.')
        except Error:
            self.ser = None
    ...
    def serverConnected(self, ID, name):
        ...
        if self.simulated:
            if (self.ser is None) and self.serNode not None and (self._matchSerial(self.serNode, name)):
                print(name, 'connected after we connected.')
                yield self.deviceSelect(None)d
        else:
    ...
    def deviceSelect(self, c, node=None, port=None, simulated=False):
        """
        ...
        try:
            ...
            if self.simulated:
                return (self.serNode, "No port (simulated).")
            else:
